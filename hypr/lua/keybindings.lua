...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 2%+ && wpctl get-volume @DEFAULT_AUDIO_SINK@ | awk '{print $2 * 100}' > $XDG_RUNTIME_DIR/wob.fifo"), { locked = true, repeating = true })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%- && wpctl get-volume @DEFAULT_AUDIO_SINK@ | awk '{print $2 * 100}' > $XDG_RUNTIME_DIR/wob.fifo"), { locked = true, repeating = true })
hl.bind("XF86AudioMute",        hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle && (wpctl get-volume @DEFAULT_AUDIO_SINK@ | grep -q 'MUTED' && echo 0 || wpctl get-volume @DEFAULT_AUDIO_SINK@ | awk '{print $2 * 100}') > $XDG_RUNTIME_DIR/wob.fifo"), { locked = true })
hl.bind("XF86MonBrightnessUp",  hl.dsp.exec_cmd("brightnessctl s 5%+ | grep -oP '\\(\\K[^%]+' > $XDG_RUNTIME_DIR/wob.fifo"), { locked = true, repeating = true })
